...
esp_err_t ws28xx_init(int pin, led_strip_model_t model, int num_of_leds,
    ...
    ws28xx_pixels = calloc(sizeof(CRGB), n_of_leds);
...
esp_err_t ws28xx_resize(int size){
    esp_err_t err = ESP_OK;
    n_of_leds = size;
    dma_buf_size = n_of_leds * 12 + (reset_delay + 1) * 2;
    /*
    esp_err_t err = ESP_OK;
    err = spi_bus_remove_device(spi_settings.spi);
    if(err != ESP_OK){
        return err;
    }
    err = spi_bus_free(spi_settings.host);
    if(err != ESP_OK){
        return err;
    }
    free(ws28xx_pixels);
    heap_caps_free(dma_buffer);
    //finished clearing old stuff

    err = ws28xx_init(spi_settings.buscfg.mosi_io_num,led_model,size,led_buffer_ptr);
